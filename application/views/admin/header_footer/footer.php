...
<!-- Dual Listbox -->
<script src="<?= base_url()?>/assets/admin/js/plugins/dualListbox/jquery.bootstrap-duallistbox.js"></script>
<script>
$(document).ready(function(){
...
function delete_page_rec(id)
{
	...
	if(delete_page_rec1==0)
	{
		...
		$.ajax({
			type       : "POST",
			data       :  {id:id,} ,
			url        : "<?= base_url()?>admin/<?= $Page_name; ?>/delete_page_rec",
			success    : function(data){
					if(data!="")
					{
...
function select_image_or_upload_load(field_name,limit) {
	...
	$.ajax({
		...
		url        : "<?= base_url()?>admin/manage_library/select_image_or_upload_api",
		success    : function(data){
			if(data!="")
			{
...
function upload_image_in_library()
{
	...
	$.ajax({
		url: "<?= base_url()?>admin/manage_library/upload_image_in_library_api",
		/*dataType: 'text',*/
		cache: false,
		contentType: false,
		processData: false,
